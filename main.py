...
async def callback_router(
    ...
    is_admin_route = (
        data != "content_list"
        and data.startswith(ADMIN_ONLY_PREFIXES)
    )
    ...
    try:
        simple_routes = {
            ...
            "channel_change_verification":
                h_admin.channel_change_verification_start,
            "channel_change_member":
                h_admin.channel_change_member_start,
            ...
            "admin_payments": h_admin.admin_stats,
        }
        ...
        if ":" in data:
            ...
            if action == "reject_payment":
                await h_upi.reject_payment_start(
                    update,
                    context,
                    int(value),
                )
            ...
            if action == "admin_user_history":
                await query.answer()
                await h_admin.admin_user_history(
                    update,
                    context,
                    int(value),
                )
            ...
            if action == "content_edit":
                await query.answer()
                await h_content.content_edit_start(
                    update,
                    context,
                    int(value),
                )
            ...
            if action == "content_delete":
                await h_content.content_delete_confirm(
                    update,
                    context,
                    int(value),
                )
            ...
            if action == "coupon_delete":
                await h_coupons.coupon_delete_confirm(
                    update,
                    context,
                    value,
                )
            ...
            if action == "coupon_toggle":
                await h_coupons.coupon_toggle_confirm(
                    update,
                    context,
                    value,
                )
        ...
        logger.exception(
            "Error handling callback data=%s from user=%s",
            data,
            user_id,
        )
...
async def generic_text_handler(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
):
